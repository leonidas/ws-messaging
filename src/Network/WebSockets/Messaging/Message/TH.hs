...
deriveMessage req n = do
    ...
    let genToJson = caseE (varE $ mkName "m") $ map conToJson cons
        genFromJson = [|
            let p (Object o) = do
                    typ <- o .: (fromString "msg")  :: Parser String
                    dat <- o .: (fromString "data") :: Parser Value
...
conFromJson req c = do
    let (name, numFields) = case c of
            ...
            _ -> error $ "Unsupported constructor type: " ++ show c
    ...
    let base = nameBase name
        ...
        body = case c of
            NormalC _ ts -> case ts of
                ...
                where bind = bindS tuple [|parseJSON $dat|]
...
conToJson c = do
    ...
    let pat = conP name $ map varP varNames
        body = normalB $ [|object [(fromString "msg") .= (base :: String), (fromString "data") .= $(dat)]|]
